<?php
    ...
    if (mysqli_select_db( $conn, $db) === false)
        ...
        if(@$_POST['btn']=="Register"){
          header("Location: http://localhost/proj/reg.php");
        }

        else if(@$_POST['btn']=="Log In"){
    ...
    if (isset($_COOKIE["user"]) && isset($_COOKIE["pass"]))
    {
        ...
        // prepare SQL
        $sql = sprintf("SELECT type as res FROM members WHERE username='%s' AND password='%s' ",
        ...
        if (mysqli_num_rows($result) == 1)
        {
			...
			echo $row["res"]."IMHERERE ";

			if($row['res']=='administrator'){
            $host = $_SERVER["HTTP_HOST"];
            $path = rtrim(dirname($_SERVER["PHP_SELF"]), "/\\");
            //$_SESSION['username'];
            header("Location: http://$host$path/admin/admin_loggedin.php");
            }

			else if($row['res']=='professor'){
            $host = $_SERVER["HTTP_HOST"];
            $path = rtrim(dirname($_SERVER["PHP_SELF"]), "/\\");
            //$_SESSION['username'];
            header("Location: http://$host$path/teacher/index.php");
            }
            else if($row['res']=='student'){
                  $host = $_SERVER["HTTP_HOST"];
                  $path = rtrim(dirname($_SERVER["PHP_SELF"]), "/\\");
                  //$_SESSION['username'];
                  header("Location: http://$host$path/student/index.php");
                  }
			exit;
    ...
    if (isset($_POST["user"]) && isset($_POST["pass"]))
    {
	    ...
	    if ($result === false)
            die("Could not query database");
		else echo "HELLLLLLLLLLLLLLLLLLLLO";
...
<div class="container log-row">
    <form class="form-signin" action="<?= $_SERVER["PHP_SELF"] ?>" method="post">
	 <div class="login-wrap">
       ...
       <br>
       <span style="color: white;"> <a href="http://localhost/proj/addcont.php"> If you wish to become a teacher or add content click here</a></span>
       <br>
	   <input name="btn" type="submit" class="btn btn-lg btn-success btn-block" value="Log In">
     <input name="btn" type="submit" class="btn btn-lg btn-success btn-block" value="Register">
     </div>
    </form>
