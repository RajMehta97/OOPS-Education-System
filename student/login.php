<?php
    ...
    if (isset($_COOKIE["user"]) && isset($_COOKIE["pass"]))
    {
        ...
        // prepare SQL
        $sql = sprintf("SELECT 1 as res FROM members WHERE username='%s' AND password='%s'",
    ...
    if (isset($_POST["user"]) && isset($_POST["pass"]))
    {
        ...
        {
			$name=$_POST["user"];
			$sql123="SELECT * FROM members WHERE username = '$name';";
	...
	/*if(isset($_POST["register"]))
	{
		   $host = $_SERVER["HTTP_HOST"];
            $path = rtrim(dirname($_SERVER["PHP_SELF"]), "/\\");
            header("Location: http://$host$path/reg.php");
...
<div class="container log-row">
    <form class="form-signin" action="<?= $_SERVER["PHP_SELF"] ?>" method="post">
	 <div class="login-wrap">
	   ...
	   <br>
	   <br>
	    <button type="submit" name="register" class="btn btn-lg btn-success btn-block"><a href="http://localhost/admin/reg.php">Register</button>
</div>	  
    </form>
